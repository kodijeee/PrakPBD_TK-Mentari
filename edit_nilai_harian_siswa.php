<?php
  ...
  $query = "SELECT * FROM nilai_harian WHERE id_nilai_harian = '".$_GET['id']."'";
  $sql = mysqli_query($connect, $query);
  $data = $sql->fetch_assoc();
  $id = $data['noinduk_siswa'];
...
<head>
  ...
  <title>Edit Nilai Harian Siswa</title>
  <!-- plugins:css -->
              ...
              <div class="card">
                <div class="card-body">
                  <div class="col-lg-12">
                    <div class="card">
                      <div class="card-body">
                        <h3 class="text-center text-primary">Edit Nilai Harian Siswa</h3>
                        <hr>
                        <br>
                        <form action="" method="post">
                          <div class="form-group">
                            <label for="id_nilai_harian">ID Nilai Harian</label>
                            <input type="text" name="id_nilai_harian" class="form-control" id="id_nilai_harian" value="<?php echo $_GET['id']; ?>" readonly required>
                          </div>
                          <div class="form-group">
                            <label for="noinduk_siswa">NIS</label>
                            <input type="text" name="noinduk_siswa" class="form-control" id="noinduk_siswa" value="<?php echo $data['noinduk_siswa']; ?>" readonly required>
                          </div>
                          <div class="form-group">
                            <label for="tgl_ambil_nilai">Tanggal Ambil Nilai</label>
                            <input type="date" name="tgl_ambil_nilai" class="form-control" id="tgl_ambil_nilai" value="<?php echo $data['tgl_ambil_nilai']; ?>" required>
                          </div>
                          <div class="form-group">
                            <button type="sumbit" name="edit" class="btn btn-small btn-primary mr-2">Save</button>
                            <a href="nilai-harian-siswa.php?id=<?php echo $data['noinduk_siswa'];?>" class="btn btn-small btn-light">Cancel</a>
                          </div>
                        </form>
                        <?php
                        ...
                        if (isset($_POST['edit'])) { // di submit atau tidak ?
                            ...
                            $tgl_ambil_nilai = $_POST['tgl_ambil_nilai'];
                        
                            // masukkan data ke database
                            $query = "UPDATE nilai_harian 
                                      SET tgl_ambil_nilai = '$tgl_ambil_nilai'
                                      WHERE id_nilai_harian = '".$_GET['id']."'";
                            $update= mysqli_query($connect, $query);
                            
                            if ($update) {
                                echo "<script>location='nilai-harian-siswa.php?id=$id';</script>";
                            } else {
