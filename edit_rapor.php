<?php
  ...
  $query = "SELECT * FROM raport WHERE id_rapor = '".$_GET['id']."'";
  $sql = mysqli_query($connect, $query);
  $data = $sql->fetch_assoc();
  $nis = $data['noinduk_siswa'];
              ...
              <div class="card">
                <div class="card-body">
                  <div class="col-lg-12">
                    <div class="card">
                      <div class="card-body">
                        <h3 class="text-center text-primary">Edit Rapor</h3>
                        <hr>
                        <br>
                        <form action="" method="post">
                          <div class="form-group">
                            <label for="id_rapor">ID Rapor</label>
                            <input type="text" name="id_rapor" class="form-control" id="id_rapor" value="<?php echo $_GET['id']; ?>" readonly maxlength="10" required>
                          </div>
                          <div class="form-group">
                            <label for="noinduk_siswa">NIS</label>
                            <input type="text" name="noinduk_siswa" class="form-control" id="noinduk_siswa" value="<?php echo $data['noinduk_siswa']; ?>" maxlength="10" readonly required>
                          </div>
                          <div class="form-group">
                            <label for="tgl_ambil_rapor">Tanggal Ambil Rapor</label>
                            <input type="date" name="tgl_ambil_rapor" class="form-control" id="tgl_ambil_rapor" maxlength="10" value="<?php echo $data['tgl_ambil_rapor']; ?>" required>
                          </div>
                          <div class="form-group">
                              <label for="cat_rapor">Catatan Rapor</label>
                              <textarea type="text" name="cat_rapor" class="form-control" id="cat_rapor" rows="6" value=""><?php echo $data['cat_rapor']; ?></textarea>
                          </div>
                          <div class="form-group">
                            <button type="sumbit" name="edit" class="btn btn-small btn-primary mr-2">Save</button>
                            <a href="rapor-siswa.php?id=<?php echo $data['noinduk_siswa'] ?>" class="btn btn-small btn-light">Cancel</a>
                          </div>
                        </form>
                        <?php
                        
                        if (isset($_POST['edit'])) { // di submit atau tidak ?
                            
                            $tgl_ambil_rapor = $_POST['tgl_ambil_rapor'];
                            $cat_rapor = $_POST['cat_rapor'];
                        
                            // masukkan data ke database
                            $query = "UPDATE raport 
                                      SET tgl_ambil_rapor = '$tgl_ambil_rapor',
                                                cat_rapor = '$cat_rapor'
                                      WHERE id_rapor = '".$_GET['id']."'";
                            $update= mysqli_query($connect, $query);
                            
                            if ($update) {
                                echo "<script>location='rapor-siswa.php?id=$nis';</script>";
                            } else {
